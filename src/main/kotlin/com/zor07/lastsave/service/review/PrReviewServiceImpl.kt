...
class PrReviewServiceImpl(
    ...
    private data class ParsedPrReview(val student: Student, val log: MessageLog, val sectionId: Long)

    @Transactional
    override fun handlePrReview(githubUsername: String, prUrl: String, diff: String) {
        try {
            val (student, log, sectionId) = parsePrReview(githubUsername)

            messageLogRepository.markPrReceived(log.id)
            logger.info("PR received for student {} in section {}", student.id, log.messageId)

            val tasksContent = materialRepository.findBySectionId(sectionId)
                .firstOrNull { it.type == MaterialType.PRACTICE }
                ?.url
                ?.let { gitHubService.fetchRawContent(it) }

            val review = aiReviewService.review(diff, tasksContent)
    ...
    private fun parsePrReview(githubUsername: String): ParsedPrReview {
        ...
        return ParsedPrReview(student, log, progress.sectionId)
    }
}
